...
# monorepo and the server imports `@godspeed/shared` as a workspace dependency;
# a build context of `server/` alone has no `shared/` to resolve it against and
# no root lockfile to install from.
#
#   docker build -t godspeed-server .
#   docker run -e PORT=2575 -p 2575:2575 godspeed-server
...
# imports require: `@godspeed/shared` resolves to `shared/dist/index.js`.
RUN npm run build:server

# Drop to production dependencies in place. This keeps the workspace symlinks
# that `@godspeed/shared` resolves through - deleting node_modules and
...
# hosts, injects one - and `serverConfig` falls back to 2575.
ENV HOST=0.0.0.0
